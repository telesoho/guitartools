...
JtabChord.prototype.parseCustomChordArrayFromToken = function () {
  ...
  var fingeredFrets = _.where(array, function (pair) { return pair.length !== 1 })
...
JtabChord.prototype.setCagedChordArray = function () {
  ...
  var cagedIndex = 'CAGED'.indexOf(this.cagedBaseShape) + 1 // get 1-based index
  var fretWidths = [3, 2, 3, 2, 2]

  var startingFret = this.cagedBaseFret

  for (var i = 1; i < this.cagedPos; i++) {
    var index = (cagedIndex - 1) % 5
    cagedIndex = (cagedIndex >= 5) ? 1 : cagedIndex + 1
    startingFret += fretWidths[index]
  }

  var modelChord = 'CAGED'.charAt(cagedIndex - 1) + this.rootExt
  this.setChordArray(modelChord)
  this.shiftChordArray(startingFret, modelChord)
}
...
JtabChord.prototype.shiftChordArray = function (atFret, modelChord) { // shift chord to new fret position
  ...
  if (atFret !== initFret) {
    var useCagedFingering = ((this.isCaged) && (this.cagedPos > 0) && (!(this.baseChords[modelChord][1][0] === undefined)))
    ...
    for (var i = 1; i < this.chordArray.length; i++) {
      ...
      var finger = (useCagedFingering) ? this.baseChords[modelChord][1][i][1] : this.chordArray[i][1]
...
Raphael.fn.increment_offset_one_chord = function (width) {
  var w = (width === undefined) ? this.chord_width : width
  if (this.debug) this.debug_grid(w)
  this.current_offset += w
  this.setSize(this.current_offset, this.chord_height)
}

Raphael.fn.svg_params = function (x, y, l1, l2) {
  // http://www.w3.org/TR/SVG/paths.html#PathData --helpful reading
  var moveLineTo = 'm' + x + ' ' + y + 'l' + l1 + ' ' + l2
  if (arguments.length === 4) return moveLineTo
}

// draw the fret board
Raphael.fn.chord_fret_board = function (position, chordName) {
  var fretLeft = this.current_offset + this.margin_left
  // conventional fret labels
  var fretLabels = ['', '', '', 'III', '', 'V', '', 'VII', '', 'IX', '', '', 'XII', '', '', 'XV', '', 'XVII', '', 'XIX', '', 'XXI', '']
  ...
  this.text( // chord name
    fretLeft + 2.5 * this.string_spacing,
    this.margin_top - 20,
    chordName).attr({ fill: this.tab_text_color, 'font-size': '10px' })

  var strokeWidth = position === 0 ? 3 : 0  // nut
  var chordFretboardPath = this.path(this.svg_params(fretLeft, this.margin_top, this.string_spacing * (this.strings_drawn - 1), 0))
  chordFretboardPath.attr({ stroke: this.color, 'stroke-width': strokeWidth })

  for (var i = 0; i <= this.frets_drawn; i++) { // frets
    this.path(this.svg_params(fretLeft, this.margin_top + (i * this.fret_spacing), this.string_spacing * (this.strings_drawn - 1), 0))

    var pos = (fretLabels[position + i] === undefined) ? '' : fretLabels[position + i]

    if (pos.length > 0) { // draw fret position
      this.text(
        fretLeft + this.fret_width + this.string_spacing * 1.0,
        this.margin_top + ((i - 0.5) * this.fret_spacing),
        pos).attr({ stroke: this.tab_text_color, 'font-size': '10px' })
    }
  }
  for (i = 0; i < this.strings_drawn; i++) {
    this.path(this.svg_params(fretLeft + (i * this.string_spacing), this.margin_top, 0, this.fret_spacing * (this.frets_drawn + 0.5)))  // strings
  }
...
Raphael.fn.stroke = function () {
  if (this.has_tab) {
    ...
    //  stroke
    var strokePath = this.path(this.svg_params(this.current_offset + this.tab_char_width, this.tab_top + (3.5 * this.tab_spacing), this.tab_char_width, -2 * this.tab_spacing))
    strokePath.attr({ stroke: this.tab_text_color, 'stroke-width': 4 })
...
Raphael.fn.bar = function () {
  if (this.has_tab) {
    ...
    // extend tab
    this.tab_extend(width)
    var barStroke = this.path(this.svg_params(this.current_offset + this.tab_char_width, this.tab_top, 0, this.tab_height))
    this.increment_offset(width)
  } else if (this.has_chord) {
    // var fretLeft = this.current_offset + this.margin_left
    barStroke = this.path(this.svg_params(this.current_offset + this.margin_left, this.margin_top, 0, 0, this.fret_height))
    this.increment_offset(this.margin_left + this.margin_right)
  }
  barStroke.attr({ stroke: this.color, 'stroke-width': 1 })
}

// draw double bar
Raphael.fn.double_bar = function () {
  if (this.has_tab) {
    var width = this.tab_char_width + 8
    // extend tab
    this.tab_extend(width)
    //  bar
    var path1 = this.path(this.svg_params(this.current_offset + this.tab_char_width, this.tab_top, 0, this.tab_height))
    var path2 = this.path(this.svg_params(this.current_offset + this.tab_char_width + 6, this.tab_top, 0, this.tab_height))
    this.increment_offset(width)
  } else if (this.has_chord) {
    var left = this.current_offset + this.margin_left

    path1 = this.path(this.svg_params(left, this.margin_top, 0, this.fret_height))
    path2 = this.path(this.svg_params(left + 6, this.margin_top, 0, this.fret_height))

    this.increment_offset(this.margin_left + 6 + this.margin_right)
  }
  path1.attr({ stroke: this.color, 'stroke-width': 1 })
  path2.attr({ stroke: this.color, 'stroke-width': 4 })
}

// draw a note in a chord
Raphael.fn.chord_note = function (position, stringNumber, note) {
  // NB: internal string_number in chords counts from low to high
  var fretNumber = note[0]
  var fretLeft = this.current_offset + this.margin_left

  if (fretNumber < 0) {
    // muted/not played
    this.text(fretLeft + (stringNumber - 1) * this.string_spacing, this.margin_top - 8, 'x').attr({ stroke: this.tab_text_color, 'font-size': '8px' })
  } else if (fretNumber === 0) {
    // open
    this.text(fretLeft + (stringNumber - 1) * this.string_spacing, this.margin_top - 8, 'o').attr({ stroke: this.tab_text_color, 'font-size': '8px' })
  } else {
    var fretDy = (fretNumber - position - 0.5) * this.fret_spacing
    // var circle =
    this.circle(
      fretLeft + (stringNumber - 1) * this.string_spacing,
      this.margin_top + fretDy, this.note_radius).attr({ stroke: this.color, fill: this.color })
    if (!(note[1] === undefined)) {
      this.text(fretLeft + (stringNumber - 1) * this.string_spacing,
        this.margin_top + fretDy, note[1]).attr({ fill: this.fingering_text_color, 'font-size': '8px' })
    }
  }

  if (this.has_tab && fretNumber >= 0) {
    this.draw_tab_note((this.strings_drawn - stringNumber + 1), fretNumber, this.margin_left + this.string_spacing * 2.5)
  }
}
...
Raphael.fn.draw_tab_note = function (stringNumber, token, leftOffset) {
  // NB: internal string_number in tab counts from high to low
  this.text(this.current_offset + leftOffset,
    this.tab_top + this.tab_spacing * (stringNumber - 1),
...
Raphael.fn.get_string_number = function (token) {
  ...
  if (token.match(/^\$[1-6]/) != null) {
    stringNumber = token.substr(1, 1)
  } else if (token.match(/^\$[EADGBe]/) != null) {
    stringNumber = 6 - 'EADGBe'.indexOf(token.substr(1, 1))
  }
  return stringNumber
}
  ...
  if (token.match(/\$/) != null) { // contains a string specifier
    if (token.match(/\./) != null) { // is a multi-string specifier
      ...
      for (i = 0; i < parts.length; i++) {
        var part = parts[i]
        var stringNumber = this.get_string_number(part)
        if (stringNumber != null) {
          this.tab_current_string = stringNumber
    ...
    var fullchordNotes = this.get_fullchord_notes(token)
    if (fullchordNotes) {
      var maxChars = fullchordNotes.max_chars()
      width = this.tab_char_width * (maxChars + 2)
      this.tab_extend(width)
      for (i = 0; i < fullchordNotes.length; i++) {
        this.draw_tab_note(6 - i, fullchordNotes[i], width * 0.5)
      }
      this.increment_offset(width)
...
Raphael.fn.render_token = function (token) {
  ...
  if (c.isValid) { // draw chord
    ...
    this.chord_fret_board(chord[0], c.chordName)
...
jtab.render = function (element, notationText) {
  var notation = notationText || element.textContent || ''
  ...
  canvasHolder.id = rndID
  canvasHolder.style.height = Raphael.fn.total_height
  element.innerHTML = ''
  element.appendChild(canvasHolder)
...
Raphael.fn.render_token_one_chord = function (token) {
  ...
  if (c.isValid) { // draw chord
    var chord = c.chordArray
    this.chord_fret_board_one_chord(chord[0], c.chordName)
    for (var i = 1; i < chord.length; i++) {
      this.chord_note_one_chord(chord[0], i, chord[i])
    }
    this.increment_offset_one_chord()
  }
}

// draw a note in a chord
Raphael.fn.chord_note_one_chord = function (position, stringNumber, note) {
  // NB: internal string_number in chords counts from low to high
  var fretNumber = note[0]
  var fretLeft = this.current_offset + this.margin_left
  ...
  if (fretNumber < 0) {
    // muted/not played
    this.text(fretLeft + (stringNumber - 1) * this.string_spacing, this.margin_top - 8, 'x').attr({
      stroke: this.tab_text_color,
      'font-size': '8px'
    })
  } else if (fretNumber === 0) {
    // open
    this.text(fretLeft + (stringNumber - 1) * this.string_spacing, this.margin_top - 8, 'o').attr({
      stroke: this.tab_text_color,
      'font-size': '8px'
    })
  } else {
    ...
    this.circle(
      fretLeft + (stringNumber - 1) * this.string_spacing,
      this.margin_top + fretDy, this.note_radius).attr({
        stroke: this.color,
        fill: this.color
      })
    if (!(note[1] === undefined)) {
      this.text(fretLeft + (stringNumber - 1) * this.string_spacing,
        this.margin_top + fretDy, note[1]).attr({
...
// draw the fret board
Raphael.fn.chord_fret_board_one_chord = function (position, chordName) {
  var stringSpacing = 8 // 弦间间隔
  var stringsDrawn = 6 // 显示的弦数量
  var fretSpacing = 10 // 音柱间隔
  var fretsDrawn = 4 // 显示的音柱数量
  // var noteRadius = 4 // 标注半径
  // var scale = 0.5
  var marginTop = 30
  // var marginBottom = 0
  var marginLeft = 5
  // var marginRight = 5
  var color = '#000'
  // var fingeringTextColor = '#fff'
  var tabTextColor = '#000'

  var fretLeft = this.current_offset + marginLeft

  var fretWidth = stringSpacing * (stringsDrawn - 1)
  // var fretHeight = fretSpacing * (fretsDrawn + 0.5)
  // var chordWidth = marginLeft + fretWidth + stringSpacing + marginRight
  // var chordHeight = marginTop + fretHeight + marginBottom

  // conventional fret labels
  var fretLabels = ['', '', '', 'III', '', 'V', '', 'VII', '', 'IX', '', '', 'XII', '', '', 'XV', '', 'XVII', '', 'XIX', '', 'XXI', '']

  this.text( // chord name
    fretLeft + 2.5 * stringSpacing,
    marginTop - 20,
    chordName).attr({
      fill: tabTextColor,
      'font-size': '10px'
  ...
  var strokeWidth = position === 0 ? 3 : 0 // nut
  var chordFretBoardPath = this.path(this.svg_params(fretLeft, marginTop, stringSpacing * (stringsDrawn - 1), 0))
  chordFretBoardPath.attr({
    stroke: color,
    'stroke-width': strokeWidth
  })

  for (var i = 0; i <= fretsDrawn; i++) { // frets
    this.path(this.svg_params(fretLeft, marginTop + (i * fretSpacing), stringSpacing * (stringsDrawn - 1), 0))

    var pos = (fretLabels[position + i] === undefined) ? '' : fretLabels[position + i]

    if (pos.length > 0) { // draw fret position
      this.text(
        fretLeft + fretWidth + stringSpacing * 1.0,
        marginTop + ((i - 0.5) * fretSpacing),
        pos).attr({
          stroke: tabTextColor,
          'font-size': '9px'
        })
  ...
  for (i = 0; i < stringsDrawn; i++) {
    this.path(this.svg_params(fretLeft + (i * stringSpacing), marginTop, 0, fretSpacing * (fretsDrawn + 0.5))) // strings
  }
}
// Render the tab for a given +element+.
// +element+ is a DOM node
// +notation_text+ is the optional notation to render (if not specified, +element+ text content will be used)
// After rendering, the +element+ will be given the additional "rendered" class.
jtab.render_one_chord = function (element, notationText) {
  // var rndID = 'builder_' + jtab.element_count++

  // add the Raphael canvas in its own DIV. this gets around an IE6 issue with not removing previous renderings
  var canvas = Raphael(element, 48, 40)
...
jtab.render_implicit = function (withinElement) {
  withinElement = withinElement || document
  _.each(withinElement.getElementsByClassName('jtab'), function (el) {
